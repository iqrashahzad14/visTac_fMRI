...
condLabelNb = [1 2 3 4 5 6 4 3 1 2 9 9]; %[1 2 3 4 5 6 4 3 1 2 5 6];
%pt fw hor ver visHor visVer % these condLabelNb the presence of only 6 categories
...
condLabelName = {'handDown_pinkyThumb', 'handDown_fingerWrist', 'handUp_pinkyThumb', 'handUp_fingerWrist',...
    ...
    'handDown_pinkyThumb', 'handDown_fingerWrist', 'handUp_pinkyThumb', 'handUp_fingerWrist',...
    'visual_horizontal','visual_vertical'};
...
for iImage = 1:length(opt.mvpa.map4D)
  ...
  for iMask = 3%1:length(opt.maskLabel)
    ...
    % load cosmo input
    ds1 = cosmo_fmri_dataset(image, 'mask', mask);
    
    ds2 = cosmo_fmri_dataset(image, 'mask', mask);
    
    %duplicate and stack the dataset
    ...
    ds = cosmo_slice(ds,ds.sa.targets ~= 9  ) ; %v to remove the conditions labelled 9 from the ds
    ...
    ds_rsm=cosmo_dissimilarity_matrix_measure(ds_mean,'center_data',true);
%     oct.metric='euclidean'; % (instead of default 'correlation')
%     ds_rsm=cosmo_dissimilarity_matrix_measure(ds_mean,oct); %default is correlation, with correlation you center the data
...
labels={'pt', 'fw', 'hor', 'ver', 'visHor','visVer'};
