...
%   condLabelNb = [1 2 3 4 5 6 ]; [1 2 1 2 5 6 ];[2 1 1 2 5 6 ];[ 1 1 2256]
  ...
  for iSub = 1:numel(opt.subjects)
    ...
    opt.maskPath = fullfile(fileparts(mfilename('fullpath')), '..','..', 'outputs','derivatives' ,'ms-roi','groupMs',strcat('sub-','ALL'));
    opt.maskName = { ...
        strcat('sub-','ALL','_hemi-L_space-MNI_label-lms_radiusNb-','cluster', '.nii'), ...
        strcat('sub-','ALL','_hemi-R_space-MNI_label-rms_radiusNb-','cluster', '.nii'), ...
        };
    
    % use in output roi name
    opt.maskLabel = {'lms', 'rms'};
